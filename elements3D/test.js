...
【微库】关于微库采集或上传到正确分类，正确的做法是 —— 采集中的输入框先输入相关的分类关键词，选择与图片符合的分类，再采集`.split(`
`);
for(var d of list){
	a = d.split('——');
	if(a.length == 2) res.push([a[0].trim(), a[1].trim()]);
}
